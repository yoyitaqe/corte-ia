...
# Instrucción: le pedimos solo lo que la IA puede leer directamente de la imagen
instruccion = """
...
# Limpiar y convertir la respuesta a datos de Python
texto_limpio = respuesta.text.strip().replace("```json", "").replace("```", "")
datos = json.loads(texto_limpio)

# Calcular la duración y si supera las 2 horas (CP>2)
try:
